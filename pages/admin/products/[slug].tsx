...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onFilesSelect = async ({ target }: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of target.files) {
        ...
        const { data } = await nextshopApi.post<{ message: string }>(
          "/admin/upload",
          formData
        );
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <TextField
              ...
              {...register("inStock", {
                required: "This field is required",
                min: { value: 0, message: "Must be a least 1 character" },
              })}
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  ...
  if (slug === "new") {
    ...
    product = await dbProducts.getProductBbySlug(slug.toString());
  }
